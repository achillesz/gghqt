...
var defaultMessage=new Array('代码/名称/拼音','输入行业名称','输入新闻关键字','输入博文关键词或博主名字')//定义一组字符窜数组
var defaultMessage1=new Array('没有可匹配的股票名称或代码','没有可匹配的板块/行业','输入新闻关键字','输入博文关键词或博主名字')
var radios_x=document.getElementsByName("whichDB");//获取radios
function wrDefault(my_obj)
{
	var messageObj=document.getElementById("textMessage")//获取目标文本框
	var change=false;
...
function submitForm_x()
{
	...
	for(var i=0; i< radios_x.length; i++)
	{
		if(radios_x[i].checked == true && hxSuggest.util.trim(submit_info)!='') submit_info = hxSuggest.submitValue;
		if(radios_x[i].checked) myNum=i;
	}
...
var hxSuggest =  function(inputObj,url,options) {
	this.input = hxSuggest.util.$(inputObj);//获取目标文本框
	this.aUrls = url;//设置urlS
	this.url = null;//设置具体URL为null
	this.setDefault(options);//设置一些属性，注意参数
...
hxSuggest.prototype = {
	...
	setDefault:function(options){
		...
		hxSuggest.util.Extend(this.options,options||{});//把参数传的options对象扩展到this.options中
	},
	//绘制悬浮面板
	drawPanel:function(){
		//存放内容
	...
	addEvent:function(){
		...
		this.input.onfocus = (function() {
			return function() {
				...
				for(var j = 0; j < radios_x.length; j++){
					radios_x[j].index = j;
					if(radios_x[j].checked == true) {
						_this.url = _this.aUrls[j];
						if(_this.url =='') return;
						_this.openSearch();
					}
				}
		...
		this.input.onblur = (function(){
			return function() {
				...
				_this.step = 0;
				for(var i = 0;i < radios_x.length; i++){
					if(radios_x[i].checked == true){
						if(_this.temp!='' && hxSuggest.util.trim(_this.input.value)!='') {
							if((!_this.common && radios_x[i].checked==true)|| _this.common) _this.input.value = _this.temp;
							if(!_this.common) {
						...
						if(hxSuggest.util.trim(_this.input.value)=='' && radios_x[i].checked == true) {
							_this.input.value = defaultMessage[i];
	...
	openSearch:function() {
		...
		this.timer = setInterval(function(){
			...
			if((value.indexOf('"')!=-1 || value.indexOf("'")!=-1 || value.indexOf("[")!=-1 || value.indexOf("]")!=-1) && value!=_this.oldValue) {
				...
				_this.setValue([]);//含有特殊字符的都传入空数组
			}
			else if(value!='' && value!=_this.oldValue) {//在value存在且改变的情况下
				_this.oldValue = value;//又把新的记录为旧的方便下一次使用
				_this.returnValue(value);//返回新的值（应该会传入URL 请求 填充）
			}
			else if(value=='') {
				_this.oldValue = value;
	...
	returnValue:function(value) {
		// console.log(value)
		if(hxSuggest.util.$(this.scriptId) && hxSuggest.util.$(this.scriptId)!=null) {//这个逻辑是不是正确？既然存在那就不能为NULL了 后面的判断多余
			var obj = hxSuggest.util.$(this.scriptId);
			obj.parentNode.removeChild(obj);//加入存在了就移除了，为下面重新请求做准备
		}
		//包装url
		var _this = this;
		var url = "";
		...
		hxSuggest.util.loadScript(url,this.scriptId,function(){
			if(hxSuggest_JsonData && hxSuggest.util.isArray(hxSuggest_JsonData)) {
				_this.conPanel.innerHTML = '';
				_this.setValue(hxSuggest_JsonData);//请求回不同的数组
			}
			else {
				_this.hide();
			}
	...
	setValue:function(d) {
		...
		var notext;
		for(var t = 0,tlen = radios_x.length; t < tlen; t++){
			if(radios_x[t].checked == true){
				notext = radios_x[t].index;
				notext = defaultMessage1[radios_x[t].index];
			}
		}
		if(d.length==0) { //如果数据为0，比方传入空数组
			str += '<tr><td style="padding:5px 0 5px 8px;">' + notext +  '</td></tr>';
			this.temp = '';
			this.tempType = '';
			if(!this.common) {
			...
			var len = Math.min(d.length,this.maxRow);//数据数，和要显示的最大数取最小
			for(var i=0;i<len;i++) {
				if(i==0) {
				...
				if(d[i].name.length>5){
					str+='<td><span title="'+d[i].name+'">'+this.redDeal(d[i].name.substr(0,4)+'..')+'</span></td>';
				}
				else {
					str+='<td>'+this.redDeal(d[i].name)+'</td>';
				}
				if(d[i].short.length>9) {
					str+='<td><span title="'+d[i].short+'">'+this.redDeal(d[i].short.substr(0,9))+'</span></td>';
				}
				else {
				...
				if(d[i].type){
					...
					str+='<td style="color:#858585;"><input type="hidden" style="display:none" value="'+d[i].stocktype+'" />'+'</td>';
					str+='</tr>';
				}
			}
		}
